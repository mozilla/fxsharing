...
import { createRef, html, ifDefined, literal, ref, repeat, staticHtml, unsafeStatic } from "./lit.all.mjs";
import { SettingElement, spread } from "./setting-element.mjs";
import MozInputFolder from "../components/moz-input-folder/moz-input-folder.mjs";
/** @import { LitElement, Ref, TemplateResult } from "./lit.all.mjs" */
/** @import { SettingElementConfig } from "./setting-element.mjs" */
...
export class SettingControl extends SettingElement {
  ...
  */
  itemsTemplate(config) {
    ...
    return repeat(itemArgs, (item) => item.config.key || item.config.id, (item) => html`<setting-control
          .config=${item.config}
          .setting=${item.setting}
          .getSetting=${this.getSetting}
          slot=${ifDefined(item.config.slot || ITEM_SLOT_BY_PARENT.get(control))}
        ></setting-control>`);
  }
  /**
  ...
  */
  optionsTemplate(config) {
    ...
    let control = config.control || "moz-checkbox";
    return repeat(config.options, (opt) => opt.key, (opt) => {
      let optionTag = opt.control ? unsafeStatic(opt.control) : KNOWN_OPTIONS.get(control);
...
if (!customElements.get("setting-control")) { customElements.define("setting-control", SettingControl); }
