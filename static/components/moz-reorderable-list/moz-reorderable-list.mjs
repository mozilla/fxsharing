...
export default class MozReorderableList extends MozLitElement {
  ...
  */
  ...
  onDragStart(event) {
    ...
    this.emitEvent(DRAGSTART_EVENT, { draggedElement });
  ...
  onDrop(event) {
    ...
    if (position === 0 && targetIndex === draggedIndex - 1 || position === -1 && targetIndex === draggedIndex + 1) {
      this.onDragEnd();
      return;
    }
    event.preventDefault();
  ...
  onDragEnd() {
    ...
    this.emitEvent(DRAGEND_EVENT, { draggedElement: this.#draggedElement });
    this.indicatorEl.hidden = true;
...
if (!customElements.get("moz-reorderable-list")) { customElements.define("moz-reorderable-list", MozReorderableList); }
