...
import { LitElement, html, ifDefined } from "../../dependencies/lit.all.mjs";
...
class ShadowDemo extends LitElement {
  ...
  static properties = { item: { type: String } };
  render() {
    return html`<style>
        #shadowed {
...
if (!customElements.get("shadow-demo")) { customElements.define("shadow-demo", ShadowDemo); }
class ReorderableDemo extends LitElement {
  ...
  get selectors() {
    switch (this.type) {
      case DEFAULT: return { itemSelector: "li" };
      case SHADOW_DOM: return { itemSelector: "#shadowed" };
      case DRAG_SELECTOR: return {
        itemSelector: "li",
        dragSelector: ".handle"
      };
      default: return {};
...
background-color: currentColor;
      ...
      >
        <ul>
          ${this.items.map((item) => html` <li>${this.contentTemplate(item)}</li> `)}
        </ul>
      </moz-reorderable-list>
...
if (!customElements.get("reorderable-demo")) { customElements.define("reorderable-demo", ReorderableDemo); }
const Template = ({ demoType }) => html`
  <reorderable-demo type=${demoType}></reorderable-demo>
`;
export const Default = Template.bind({});
Default.args = { demoType: DEFAULT };
export const ShadowDOM = Template.bind({});
ShadowDOM.args = { demoType: SHADOW_DOM };
export const DragSelector = Template.bind({});
DragSelector.args = { demoType: DRAG_SELECTOR };
