  ...
  */
  #register() {
    if (window.document.nodePrincipal?.isSystemPrincipal) {
      ChromeUtils.defineESModuleGetters(MozSupportLink, { BrowserUtils: "resource://gre/modules/BrowserUtils.sys.mjs" });
      // eslint-disable-next-line no-shadow
      let { XPCOMUtils } = window.XPCOMUtils ? window : ChromeUtils.importESModule("resource://gre/modules/XPCOMUtils.sys.mjs");
  ...
  connectedCallback() {
    ...
    if (!this.getAttribute("data-l10n-id") && !this.getAttribute("data-l10n-name") && !this.childElementCount) {
      if (!this.textContent) { this.textContent = "Learn more"; }
      if (document.l10n) { document.l10n.setAttributes(this, "moz-support-link-text"); }
    }
    if (document.l10n) { document.l10n.translateFragment(this); }
  }
...
if (!customElements.get("moz-support-link")) { customElements.define("moz-support-link", MozSupportLink, { extends: "a" }); }
/**
...
export function formatUTMParams(contentAttribute, url) {
  ...
  if (!domain.endsWith(".mozilla.org") && !domain.endsWith(".allizom.org")) {
    return url;
  }
