...
export default class MozButtonGroup extends MozLitElement {
  ...
  static properties = { platform: { state: true } };
  constructor() {
    super();
    this.#detectPlatform();
  }
  ...
  render() {
    let slots = [html` <slot @slotchange=${this.onSlotchange}></slot> `, html` <slot name="primary"></slot> `];
    if (this.platform == PLATFORM_WINDOWS) {
  ...
  static styles = [...MozLitElement.styles ?? [], css`/* From chrome://global/content/elements/moz-button-group.css */
:host {
  display: flex;
...
::slotted(button:not(:first-child, .popup-notification-dropmarker)), ::slotted(moz-button:not(:first-child)) {
  margin-inline-start: var(--space-small) !important;
}

`];
}
if (!customElements.get("moz-button-group")) { customElements.define("moz-button-group", MozButtonGroup); }
