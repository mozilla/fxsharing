...
import { html, ifDefined, classMap, nothing } from "../../dependencies/lit.all.mjs";
...
const AVATAR_ICONS = [
  ...
  new URL("../../assets/experiments.svg", import.meta.url).href,
...
function getSlottedContent(type, index) {
  ...
  return html`<div class="slotted avatar">
    <acorn-icon src=${AVATAR_ICONS[index]} role="presentation"></acorn-icon>
  </div>`;
}
const Template = ({ value, slottedItem, pickerL10nId, supportPage, type, showItemLabels, imageSrc, showItemDescriptions }) => {
  return html`
    <style>
      .slotted {
        display: flex;
...
color: var(--icon-color);
    ...
    >
      ${[...Array.from({ length: 3 })].map((_, i) => html`<moz-visual-picker-item
            value=${i + 1}
            class=${classMap({ "avatar-item": slottedItem == "avatar" })}
            data-l10n-id=${slottedItem == "avatar" ? AVATAR_L10N_IDS[i] : nothing}
            label=${showItemLabels ? `Item number ${i + 1}` : nothing}
            description=${showItemDescriptions ? `Description for item number ${i + 1}` : nothing}
