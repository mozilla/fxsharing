...
import { html, LitElement, classMap } from "../../dependencies/lit.all.mjs";
import { tokensTable, variableLookupTable } from "../../dependencies/tokens-table.mjs";
import styles from "../../dependencies/tokens-table.css?url";
export default {
...
class TablesPage extends LitElement {
  ...
  handleSearch() {
    ...
    let filteredTokens = Object.entries(this.tokensData).reduce((acc, [key, tokens]) => {
      ...
      let filteredItems = tokens.filter(({ name: tokenName, value }) => this.filterTokens(this.#query, tokenName, value));
...
if (!customElements.get("tables-page")) { customElements.define("tables-page", TablesPage); }
/**
*
*/
class TokensTable extends LitElement {
  ...
  getDisplayProperty(category) {
    switch (category) {
      case "attention-dot":
      case "color":
      case "box-shadow": return "background-color";
      case "text-color": return "color";
      default: return category.replace("button", "");
    }
  }
    ...
    return html`
      <td>
        <div class="preview-wrapper">
          ${type == "preview" ? html`${this.getTemplate(raw ?? value, tokenName)}` : html`<p class="value">${value}</p>
...
if (!customElements.get("tokens-table")) { customElements.define("tokens-table", TokensTable); }
