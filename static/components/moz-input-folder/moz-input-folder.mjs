...
import { html, classMap, styleMap, css } from "../../dependencies/lit.all.mjs";
import MozInputText from "../moz-input-text/moz-input-text.mjs";
/**
...
export default class MozInputFolder extends MozInputText {
  ...
  static queries = { chooseFolderButtonEl: "#choose-folder-button" };
  constructor() {
    super();
    this.readonly = true;
    this.displayValue = "";
  ...
  getInputIconSrc(folder) {
    if (!folder || !this.hasServices()) {
      let defaultIconSrc = "../../assets/folder.svg";
  ...
  inputTemplate() {
    ...
    if (this._inputIconSrc) {
      classes = classMap({ "with-icon": true });
      styles = styleMap({ "--input-background-icon": `url(${this._inputIconSrc})` });
    }
    return html`
      <div class="container">
        ${super.inputTemplate({
  ...
  static styles = [...MozInputText.styles ?? [], css`/* From chrome://global/content/elements/moz-input-folder.css */
@import "../../dependencies/moz-input-text.css";

:host {
  --input-folder-space-inline: var(--space-medium);
  --input-text-icon-size: var(--icon-size);
  width: 100%;
}
...
if (!customElements.get("moz-input-folder")) { customElements.define("moz-input-folder", MozInputFolder); }
