...
`moz-toggle` can be imported into `.html` files:

```html
<script type="module" src="./moz-toggle.ts"></script>
```
...
<moz-toggle
  ...
  aria-label="Toggle label if label text isn't visible"
></moz-toggle>
```
...
```html
<moz-toggle
  data-l10n-id="with-label-and-description"
